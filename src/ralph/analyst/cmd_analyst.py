    ...
    def load_catalogue_data(self):
        ...
        if self.file_path is not None:
            if (self.separator != None):
                self.log.debug(f'CMD Analyst: Loading catalogue from file with specified separator: {self.separator}')
                data, labels = self.load_catalogue_from_file_data(separator=self.separator)
            else:
                self.log.debug(f'CMD Analyst: Loading catalogue from file with default separator: {self.separator}')

...
class LogsTest:
    ...
    def test_create_log(self):
        ...
        self.log = logs.start_log(
            os.path.join("tests", "microlensing_ralph", "data", "output", "logs"),
            self.log_level,
...
def test_run():
    ...
    analyst_path = os.path.join("tests", "microlensing_ralph", "data", "output", "logs")
    log_level = ["info", "debug", "error"]

    for level in log_level:
        fpath = os.path.join(analyst_path, f"test_{level:s}_analyst.log")
        output = Path(fpath)
        if output.exists():
            os.remove(output)
